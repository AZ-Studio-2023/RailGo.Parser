...
class MongoSQLiteExporter(MongoJsonExporter):
    def clear(self):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS stations(telecode TEXT PRIMARY KEY NOT NULL,telecodeAlias TEXT,pinyin TEXT NOT NULL,pinyinTriple TEXT NOT NULL,tmism TEXT,tmismAlias TEXT,name TEXT NOT NULL,level TEXT,bureau TEXT,belong TEXT,province TEXT,city TEXT,sameCityStations TEXT,lines TEXT,type TEXT,trainList TEXT);")
        db.commit()
    ...
    def export(self):
        ...
        for d in self.stationInfoList():
            try:
                cursor.execute("INSERT INTO stations (telecode, telecodeAlias, pinyin, pinyinTriple, tmism, tmismAlias, name, level, bureau, belong, province, city, sameCityStations, lines, type, trainList) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            (d["telecode"], self._serializeJSON(d["telecodeAlias"]), d["pinyin"], d["pinyinTriple"], d["tmism"], self._serializeJSON(d["tmismAlias"]), d["name"], d["level"], d["bureau"], d["belong"], d["province"], d["city"], self._serializeJSON(d["sameCityStationList"]), self._serializeJSON(d["lines"]), self._serializeJSON(d["type"]), self._serializeJSON(d["trainList"])))
                db.commit()
            except Exception as e:
                print(e)
                db.rollback()
